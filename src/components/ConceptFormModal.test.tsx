...
describe("ConceptFormModal prerequisites (#118)", () => {
  ...
  it("edit 保存時に ContextCard provenance を落とさない", async () => {
    const user = userEvent.setup();
    const onSubmit = vi.fn(async (payload) => {
      expect(payload.sourceContextCardId).toBe("card-1");
      expect(payload.autoGenerated).toBe(true);
      expect(payload.sourceContextCardTermKey).toBe("ai");
      expect(payload.title).toBe("人工知能");
      return undefined;
    });
    render(
      <ConceptFormModal
    ...
    await user.type(titleInput, "人工知能");
    await user.click(screen.getByRole("button", { name: "保存" }));
    await waitFor(() => expect(onSubmit).toHaveBeenCalled());
  });
});
